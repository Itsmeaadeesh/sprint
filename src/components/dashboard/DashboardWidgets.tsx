...
export const DashboardWidgets: React.FC<DashboardWidgetsProps> = ({ onQuickAdd: _onQuickAdd }) => {
  ...
  return (
    <div className="px-2.5 sm:px-6 py-2 editorial-border-b bg-[var(--bg)] font-mono text-xs select-none w-full overflow-hidden">
      <div className="flex items-center justify-between gap-2">
        {!collapsed ? (
          <div className="flex items-center gap-3 sm:gap-6 overflow-x-auto no-scrollbar py-0.5 max-w-full touch-pan-x">
            {/* Progress metric */}
            ...
            {highPriorityTasks.length > 0 && (
              <div className="flex items-center gap-1.5 text-[var(--accent)] flex-shrink-0">
          ...
          <div className="flex items-center gap-2 text-[10px] sm:text-[11px] font-bold uppercase text-[var(--muted)]">
            <span>{openTasks.length} open</span>
            <span>·</span>
            <span>{completionPct}% resolved</span>
          </div>
        )}

        <button
          onClick={() => setCollapsed(!collapsed)}
          aria-label={collapsed ? 'Expand dashboard metrics' : 'Collapse dashboard metrics'}
          className="text-[var(--muted-3)] hover:text-[var(--fg)] p-2 cursor-pointer editorial-border min-w-[34px] min-h-[34px] flex items-center justify-center flex-shrink-0"
        >
